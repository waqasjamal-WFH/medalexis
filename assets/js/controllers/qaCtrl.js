...
app.controller('addqaCtrl', ["$scope","$location","$http","$localStorage","toaster" ,"$state", "$stateParams",function($scope,$location,$http,$localStorage,toaster, $state,$stateParams ){
    if($localStorage.user_data['user_permission'][0].add_QA== 1){	
...
app.controller('listqaCtrl', ["$scope", "$filter", "ngTableParams","$uibModal", "$log", "$localStorage","$location","$http","toaster","$rootScope","$timeout" ,"$state", "$stateParams", function ($scope, $filter, ngTableParams, $uibModal, $log, $localStorage, $location ,$http, toaster, $rootScope, $timeout,$state, $stateParams ) {
    if($localStorage.user_data['user_permission'][0].list_QA== 1){
        ...
        if($rootScope.opentoastqadelete== "1"){
        ...
        $scope.openmodeldeleteqa = function (uid) {
     
         $scope.qaid= uid;
         // console.log($scope.userid);
          var modalInstance = $uibModal.open({

            templateUrl: 'myModalContentdeleteqa.html',
            controller: 'ModalUiCtrlqadelete',
            scope : $scope,
            size: 'sm',
            backdrop: 'static',
            resolve: {
...
app.controller('ModalUiCtrlqa', ["$scope", "$rootScope", "$uibModalInstance", "items","$http","$location","PDFKit","$sce","$localStorage","toaster","$state", "$stateParams", function ($scope, $rootScope, $uibModalInstance, items,$http, $location,PDFKit,$sce,$localStorage, toaster, $state , $stateParams) {
  ...
  var param={
    ...
    $http.post($location.protocol()+"://"+$location.host()+"/medilixis_server/public/getselectedqa", param)
    .then(function(response) {
      if(response.data.status=="success"){
        var selected_access=[];
...
app.controller('ModalUiCtrlqadelete', ["$scope", "$rootScope", "$uibModalInstance", "items","$http","$location","PDFKit","$sce","$localStorage" ,"toaster","$state", "$stateParams", function ($scope, $rootScope, $uibModalInstance, items,$http, $location,PDFKit,$sce,$localStorage , toaster, $state , $stateParams) {
  ...
  //....................on click ok button on assigning qa and transcriber model id inserted to mysql table task_ permission START....////
    $scope.ok = function () {
        ...
        var data= {"token":$localStorage.user_data.response.token , "uid":$scope.qaid 
        ...
        .then(function(response) {
           ...
           if(response.data.status=="success"){
                ...
                $scope.toaster = {
                  type: 'error',
                  title: 'Unsuccessful',
                  text: 'Error deleting QA'
                };
                return toaster.pop($scope.toaster.type, $scope.toaster.title,$scope.toaster.text);
            }
        }).catch(function(){
              console.log("Error deleting QA");
        }); 
    };
