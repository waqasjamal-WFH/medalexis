...
app.controller('addreceptionistCtrl', ["$scope","$location","$http","$localStorage","toaster",function($scope,$location,$http,$localStorage,toaster){
    ...
    $scope.access_right=[
     ...
     $scope.submit= function(){
        ...
        $http.post($location.protocol()+"://"+$location.host()+"/medilixis_server/public/addreceptionist", param)
        .then(function(response) {
          // console.log(response.data);
           if(response.data.status=="success"){
                $scope.toaster = {
...
app.controller('listreceptionistCtrl', ["$scope", "$filter", "ngTableParams", "$uibModal", "$log","$localStorage","$location","$http","toaster","$rootScope","$timeout", function ($scope, $filter, ngTableParams, $uibModal, $log, $localStorage, $location ,$http, toaster, $rootScope, $timeout) {
    ...
    //................................http post request for getting company list start here........................
        // var datas=[];
        var param={'token' :$localStorage.user_data.response.token};
        $http.post($location.protocol()+"://"+$location.host()+"/medilixis_server/public/getreceptioniest", param)
        .then(function(response) {
          // console.log(response.data);
          if(response.data.status=="success"){
    ...
    $scope.openmodelrecep = function (uid) {
        $scope.recepuserid= uid;
      // $scope.taskid=taskId
      // console.log(taskId);
    
      var modalInstance = $uibModal.open({

        templateUrl: 'myModalContentrecep.html',
        controller: 'ModalUiCtrlrecep',
        scope : $scope,
        size: 'lg',
        backdrop: 'static',
        resolve: {
          items: function () {
            return $scope.recepuserid;
...
app.controller('ModalUiCtrlrecep', ["$scope", "$rootScope", "$uibModalInstance", "items","$http","$location","PDFKit","$sce","$localStorage","toaster","$state", "$stateParams", function ($scope, $rootScope, $uibModalInstance, items,$http, $location,PDFKit,$sce,$localStorage, toaster, $state , $stateParams) {
  ...
  var param={
            ...
            'uid': $scope.recepuserid
    };

    $http.post($location.protocol()+"://"+$location.host()+"/medilixis_server/public/getselectedreceptionest", param)
    .then(function(response) {
      if(response.data.status=="success"){
        var selected_access=[];
