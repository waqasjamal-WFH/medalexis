...
app.controller('addpracticeCtrl', ["$scope","$location","$http","$localStorage","toaster" ,"$state", "$stateParams",function($scope,$location,$http,$localStorage,toaster, $state, $stateParams ){
    if($localStorage.user_data['user_permission'][0].add_practiceadmin== 1){
        ...
        $scope.submit= function(){
            ...
            $http.post($location.protocol()+"://"+$location.host()+"/medilixis_server/public/addpracticeadmin", param)
            .then(function(response) {
              // console.log(response.data);
               if(response.data.status=="success"){
                    $scope.toaster = {
                      type: 'success',
                      title: 'Successful',
                      text: 'Practice Admin Added Successfully'
                    };
                    $location.path('app/listpracticeadmin');
                    return toaster.pop($scope.toaster.type, $scope.toaster.title,$scope.toaster.text);
                }else{
                    $scope.toaster = {
                      type: 'error',
                      title: 'Unsuccessful',
                      text: 'Error adding Practice Admin'
                    };
                    return toaster.pop($scope.toaster.type, $scope.toaster.title,$scope.toaster.text);
                }
            }).catch(function(){
                  console.log("Error adding Practice Admin");
            }); 
        }
    }else{
...
app.controller('listpracticeCtrl', ["$scope", "$filter", "ngTableParams","$uibModal", "$log","$localStorage","$location","$http","toaster","$rootScope","$timeout","$state", "$stateParams", function ($scope, $filter, ngTableParams, $uibModal, $log, $localStorage, $location ,$http, toaster, $rootScope, $timeout, $state, $stateParams) {
    if($localStorage.user_data['user_permission'][0].list_practiceadmin== 1){
        ...
        //................................http post request for getting company list start here........................
            // var datas=[];
            var param={'token' :$localStorage.user_data.response.token};
            $http.post($location.protocol()+"://"+$location.host()+"/medilixis_server/public/getpracticeadmin", param)
            .then(function(response) {
              // console.log(response.data);
              if(response.data.status=="success"){
        ...
        $scope.openmodelpractice = function (uid) {
            $scope.practiceuserid= uid;
          // $scope.taskid=taskId
          // console.log(taskId);
...
app.controller('ModalUiCtrltrancoadmin', ["$scope", "$rootScope", "$uibModalInstance", "items","$http","$location","PDFKit","$sce","$localStorage","toaster","$state", "$stateParams", function ($scope, $rootScope, $uibModalInstance, items,$http, $location,PDFKit,$sce,$localStorage, toaster, $state , $stateParams) {
  ...
  var param={
            ...
            'uid': $scope.practiceuserid
    };

    $http.post($location.protocol()+"://"+$location.host()+"/medilixis_server/public/getselectedpracticeadmin", param)
    .then(function(response) {
      if(response.data.status=="success"){
        var selected_access=[];
