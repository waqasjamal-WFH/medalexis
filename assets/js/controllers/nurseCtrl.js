...
app.controller('addnurseCtrl', ["$scope","$location","$http","$localStorage","toaster" ,"$state", "$stateParams",function($scope,$location,$http,$localStorage,toaster, $state, $stateParams ){
    if($localStorage.user_data['user_permission'][0].add_nurse== 1){	
...
app.controller('listnurseCtrl', ["$scope", "$filter", "ngTableParams","$uibModal", "$log", "$localStorage","$location","$http","toaster","$rootScope","$timeout" ,"$state", "$stateParams", function ($scope, $filter, ngTableParams,$uibModal,$log, $localStorage, $location ,$http, toaster, $rootScope, $timeout,$state,$stateParams ) {
    if($localStorage.user_data['user_permission'][0].list_nurse== 1){
        if($rootScope.opentoastnurse== "1"){
        ...
        //................................http post request for getting nurse list start here........................
            // var datas=[];
            var param={'token' :$localStorage.user_data.response.token};
            $http.post($location.protocol()+"://"+$location.host()+"/medilixis_server/public/getnurse", param)
            .then(function(response) {
              // console.log(response.data);
              if(response.data.status=="success"){
        ...
        //.............................http post request for getting nurse list end here...............................
...
app.controller('ModalUiCtrlnurse', ["$scope", "$rootScope", "$uibModalInstance", "items","$http","$location","PDFKit","$sce","$localStorage","toaster","$state", "$stateParams", function ($scope, $rootScope, $uibModalInstance, items,$http, $location,PDFKit,$sce,$localStorage, toaster, $state , $stateParams) {
  ...
  //....................on click ok button on assigning qa and transcriber model id inserted to mysql table task_ permission START....////
    $scope.ok = function () {
        var data= {"token":$localStorage.user_data.response.token , "userID":$scope.nurseuserid ,
        ...
        $http.post($location.protocol()+"://"+$location.host()+"/medilixis_server/public/editselectednurse", data)
            .then(function(response) {

               if(response.data.status=="success"){
                    $rootScope.opentoastnurse= "1";
                    ...
                    $scope.toaster = {
                      type: 'error',
                      title: 'Unsuccessful',
                      text: 'Error editing nurse'
                    };
                    return toaster.pop($scope.toaster.type, $scope.toaster.title,$scope.toaster.text);
                }
            }).catch(function(){
                  console.log("Error Editing nurse");
            });
    };
